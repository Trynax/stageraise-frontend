...
export function FundersList({ contributions, token }: FundersListProps) {
  ...
  const handleScroll = () => {
    ...
    if (isNearBottom && displayCount < contributions.length) {
      setDisplayCount(prev => Math.min(prev + 20, contributions.length))
    }
  }
  ...
  return (
    <div className="bg-white border-2 border-dark rounded-2xl p-6">
      ...
      <div 
        ref={scrollRef}
        className="space-y-3 max-h-[500px] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
      >
          ...
          <>
            ...
            {displayCount < contributions.length && (
              <div className="text-center py-4 text-sm text-gray-500">
                Loading more... ({displayCount} of {contributions.length})
              </div>
            )}
          </>
        ) : (
